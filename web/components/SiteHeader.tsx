...
export function SiteHeader({
  ...
  return (
    <>
      <header className="fixed inset-x-0 top-0 z-50 border-b border-amber-600/80 bg-zinc-50 md:sticky dark:bg-zinc-950">
          ...
          {menuOpen ? (
            <>
              <button
                ...
                className="fixed inset-0 z-40 cursor-pointer bg-zinc-950/50 md:hidden"
                type="button"
                onClick={closeMenu}
              />
              <nav
                aria-label="Mobile"
                className="fixed inset-x-0 top-16 bottom-0 z-50 flex flex-col gap-3 overflow-y-auto bg-zinc-50 px-5 pb-4 pt-2 sm:px-6 md:hidden dark:bg-zinc-950"
                id={menuId}
              >
                <NavItems
      ...
      <div aria-hidden="true" className="h-16 shrink-0 md:hidden" />
    </>
  );
}
